module board_top #(
    parameter   BUILD_DATE      = 32'h20231107,
...
always @(posedge FPGA_50MHz)begin
    ...
    RX_LOCKN    <=  rx_lockn;
    RX_HPDN     <=  rx_hpdn;
    LED[0]      <=  rx_lockn | rx_hpdn      ? 1 : 0;      //active low,  LED low On
    LED[1]      <=  rx_trn_done             ? 0 : 1;      //active high, LED low On
    LED[2]      <=  tx_lockn | tx_hpdn      ? 1 : 0;      //active low , LED low On
end
...
g_reset vx1_reset(
    ...
    .gterr              (VIDEO_TIMING== "4K" ? gt_rxdisperr[16*4-1:0]     :gt_rxdisperr ),
    .gtnit              (VIDEO_TIMING== "4K" ? gt_rxnotintable[16*4-1:0]  :gt_rxnotintable ),
