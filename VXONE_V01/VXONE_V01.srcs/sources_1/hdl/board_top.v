module board_top #(
    parameter   BUILD_DATE      = 32'h20231123,
    parameter   SIMULATION      = 0,
    parameter   CH_NUM          = 32,  // vx1 channel number
    parameter   DW              = 32,  // vx1 link data width
    parameter   CW              = 16,
    parameter   RX_RVS          = 1,
    parameter   VIDEO_TIMING    = "4K",     // "4K" or "8k"
//    parameter   VIDEO_TIMING    = "8K",     // "4K" or "8k"
//    parameter   VX1_PCLK_RATE   = "74p25")  // "89p10","84p24" or "74p25"
    parameter   VX1_PCLK_RATE   = "84p24")  // "89p10","84p24" or "74p25"
//    parameter   VX1_PCLK_RATE   = "89p10")  // "89p10","84p24" or "74p25"
(   FPGA_50MHz, // freerun clock
    ...
    RX_LOCKN,           RX_HPDN,
    TX_LOCKN,           TX_HPDN,
    RX_DATA_FORMAT,RX_AC_DET,RX_RSVD,RX_RO_BYPASS,
    RX_VX1_MODE,RX_OPTION,RX_WP_IN,RX_QSMEN,
    RX_3D_EN,RX_C_SDA,RX_C_SCL,RX_V_SDA,RX_V_SCL,
    RX_RS_DONE,RX_ERR_DET,RX_RF_DONE,
    TX_DATA_FORMAT,TX_AC_DET,TX_RSVD,TX_RO_BYPASS,
    TX_VX1_MODE,TX_OPTION,TX_WP_IN,TX_QSMEN,
    TX_3D_EN,TX_C_SDA,TX_C_SCL,TX_V_SDA,TX_V_SCL);
...
//assign  RX_DATA_FORMAT= RX_DATA_FORMAT;
//assign  RX_OPTION     = RX_OPTION;
//assign  RX_RSVD       = RX_RSVD;
//assign  RX_RO_BYPASS  = RX_RO_BYPASS;
//assign  RX_VX1_MODE   = RX_VX1_MODE;
//assign  RX_QSMEN      = RX_QSMEN;
//assign  RX_WP_IN      = RX_WP_IN;
//assign  RX_C_SDA      = RX_C_SDA;
//assign  RX_C_SCL      = RX_C_SCL;
//assign  RX_V_SDA      = RX_V_SDA;
//assign  RX_V_SCL      = RX_V_SCL;
//assign  RX_3D_EN      = RX_3D_EN;
//assign  RX_AC_DET     = RX_AC_DET;
//assign  RX_RS_DONE    = RX_RS_DONE;
//assign  RX_ERR_DET    = RX_ERR_DET;
//assign  RX_RF_DONE    = RX_RF_DONE;
...
always @(posedge FPGA_50MHz)begin
    ...
    RX_LOCKN    <=  rx_lockn;
    RX_HPDN     <=  rx_hpdn;
    LED_R[0]      <=  rx_lockn | rx_hpdn      ? 1 : 0;      //active low,  LED low On
    LED_R[1]      <=  rx_trn_done             ? 0 : 1;      //active low,  LED low On
    LED_R[2]      <=  tx_lockn | tx_hpdn      ? 1 : 0;      //active low , LED low On
end
wire rx_usrclk;
wire tx_usrclk;
assign LED[0] = rx_usrclk;
assign LED[1] = tx_usrclk;
...
wire ref_clk_74p25;
//generate
//if(VX1_PCLK_RATE == "89p10")begin : rxrec89p10
//    clk_wiz_0 clk_wiz_0(.clk_out1(ref_clk_74p25),.clk_in1(rx_usrclk),.locked(mmcm_lock),.reset(vx1_rst));
//end
//if(VX1_PCLK_RATE == "84p24")begin : rxrec84p24
//    clk_wiz_0 clk_wiz_0(.clk_out1(ref_clk_74p25),.clk_in1(rx_usrclk),.locked(mmcm_lock),.reset(vx1_rst));
//end
//endgenerate
...
assign ref_clk =  vio_cable_loopback    ? pll_refclk_74p25 :
                  ...
                  RX_NG                 ? pll_refclk_74p25 :
                  mmcm_lock             ? ref_clk_74p25:
                  rx_usrclk;
...
assign gt_err = VIDEO_TIMING == "4K" ? |gt_rxdisperr[16*4-1:0] | |gt_rxnotintable[16*4-1:0] :
...
vx1_link#(
    ...
    .CH_NUM(CH_NUM),
    .DW(DW),
    .VIDEO_TIMING(VIDEO_TIMING)) vx1_link(
...
vx1_debug #(
.VX1_PCLK_RATE(VX1_PCLK_RATE),
.VIDEO_TIMING(VIDEO_TIMING))
vx1_debug(
    .vio_gt_loopback(vio_gt_loopback),  .vio_tx_tpg_en  ((vio_tx_tpg_en|RX_NG)),    .vio_rx_ch_sel(vio_rx_ch_sel),
